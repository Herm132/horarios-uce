...
class AsignaturaSerializer(serializers.ModelSerializer):
    class Meta:
        model = Asignatura
        fields = ["id_asignatura", "nombre", "codigo", "semestre", "es_comun"]


class SemestreLectivoSerializer(serializers.ModelSerializer):
    class Meta:
        model = SemestreLectivo
        fields = ["id_semestre_lectivo", "anio_inicio", "anio_fin", "periodo"]
...
class HorarioSerializer(serializers.ModelSerializer):
    ...
    semestre_lectivo = SemestreLectivoSerializer(
        source="id_semestre_lectivo", read_only=True
    )

    id_asignatura = serializers.PrimaryKeyRelatedField(
        queryset=Asignatura.objects.all(), write_only=True
    )
    id_aula = serializers.PrimaryKeyRelatedField(
        queryset=Aula.objects.all(), write_only=True
    )
    id_hora_clase = serializers.PrimaryKeyRelatedField(
        queryset=HoraClase.objects.all(), write_only=True
    )
    id_usuario = serializers.PrimaryKeyRelatedField(
        queryset=Usuario.objects.all(), write_only=True
    )
    id_semestre_lectivo = serializers.PrimaryKeyRelatedField(
        queryset=SemestreLectivo.objects.all(), write_only=True
    )
    ...
    def validate(self, data):
        ...
        if id_asignatura.es_comun and not id_aula.uso_general:
            raise serializers.ValidationError(
                "Las asignaturas comunes solo pueden ser asignadas a aulas de uso general."
            )
